...
/** Einheiten wie im Backend/Types */
...
export default function NewDeviceModal({ open, onClose, onCreated }: Props) {
  ...
  // NEU: Zusatzfelder
  const [baujahr, setBaujahr] = useState<string>("");                 // Zahl (optional)
  const [vermietetIn, setVermietetIn] = useState<string>("");         // ISO-Land (optional), z.B. "DE"
  const [mietpreis, setMietpreis] = useState<string>("");             // Zahl (optional)
  ...
  async function submit() {
    ...
    try {
      await api.createGeraet({
        ...
        // NEU: nur senden, wenn sinnvoll belegt – sonst null
      ...
      setBaujahr(""); setVermietetIn(""); setMietpreis(""); setMietEinheit("MONATLICH");
    } catch (e: any) {
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="flex flex-col gap-1">
              ...
              <select className="rounded-xl border p-2" value={status} onChange={e => setStatus(e.target.value as any)}>
